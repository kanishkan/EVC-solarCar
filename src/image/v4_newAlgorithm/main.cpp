	...
	if(horizLane== -1 || discardHorizLane ==1){		// Only vertical Lanes
		if(leftLane!=-1 && rightLane!=-1){		// Two Lanes
			...
			if(leftLane!=-1){	// Left Lane alone
				...
				angleOffset = 45-lanes_org[rightLane].angle;
				//p2.x = leftLaneHighPt;
				//p2.y = 0;
				...
				printf("Forbidden Case.\n" );
				currentFrame.lanePatern = ERROR;
			}
		}
		...
		if(leftLane!=-1 && rightLane!=-1){			// Two vertical and one horizontal
			...
			if(leftLane!=-1){	// Left Lane alone
				...
				printf("Only Horizontal Lanes.! Decide Control based on previous values.!\n" );
...
//	else if(currentFrame.lanePatern == HORIZONTAL){	
	...
	if(validPrevFame==1 && currentFrame.lanePatern ==HORIZONTAL)
	{
		...
		*/
		if(previousFrame.nextDirection == !NONE){
			...
			printf(" Initial Condition is Horizontal Lane.\n");
		}
	}else if(currentFrame.lanePatern ==HORIZONTAL){
		// Choose based on angle(angle == y position)
		if(p2.y>p1.x){
	...
	if(currentFrame.lanePatern == UNKNOWN){
	...
	P2_tmp.x = p2.x ;//(int)round(P1_tmp.x + length * cos(angle * CV_PI / 180.0));
	P2_tmp.y = p2.y ;//(int)round(P1_tmp.y - length * sin(angle * CV_PI / 180.0));
	...
	printf("Correction: %f Porjection Angle: %f, angleOffset: %f\n",error,angle,angleOffset);
