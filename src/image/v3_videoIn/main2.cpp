...
void findLane(vector<Vec4i> *lines, Mat edge, Mat original, Mat raw);
void filterLane(std::vector<Lane> lanes, Mat edges, Mat raw);
void findPosition(std::vector<LanePair> L_Pair, Mat edges, Mat raw);
...
//#elseif VIDEO_FILE
#elif VIDEO_FILE
	VideoCapture cap("C:\\Users\\s131149\\Downloads\\video.h264");
	if(!cap.isOpened()){  // check if we succeeded
	...
	while(1)
	{
		 waitKey(1000);
		 Mat rawImg;
		 for(int fRate=0;fRate<30;fRate++)
			cap >> rawImg;
		if(rawImg.empty())
            break;
    ...
    if ( argc == 2 ){
		...
		char imgeLocation[100] = "..\\vid_case\\test3.png";
	...
	}
{
	...
	// Lane detection
	findLane(&lines,im_edge,im_roi,image);
}
	waitKey(0);
}
...
void filterLane(std::vector<Lane> lanes_org, Mat edges, Mat raw){
	int w = edges.cols;
	int h = edges.rows;
	...
	// Config
	float m_deviation = 1, c_deviation_min=8, c_deviation_max=40;
	for(int i=0;i<lanes.size(); i++)
		votes[i] = 0;
...
void findPosition(std::vector<LanePair> L_Pair, Mat edges, Mat raw){
	...
	if(leftLaneCount==0 && rightLaneCount==0 && horizLaneCount==0)
		sprintf (result,"No Lane found");
	else if(leftLaneCount>0 && rightLaneCount>0){			// Two lanes
		leftBorder = leftLane[leftLaneCount-1];
		rightBorder = rightLane[rightLaneCount-1];
		if(horizLaneCount == 0){	// No horizontal lane
			sprintf (result,"Direction: Forward  (Logic: Two Verical Lanes)");
			
			if( fabs(leftBorder.m+rightBorder.m) <= 1 ){	// Both has same angle. 
				sprintf(result,"Control: No-change");
			}
			else if(fabs(leftBorder.m) >= fabs(rightBorder.m)){	// Inclined towards left
				sprintf(result,"Control: %f degree-Right",fabs(leftBorder.angle)-fabs(rightBorder.angle));
			}
			else{
				sprintf(result,"Control: %f degree-Left",fabs(rightBorder.angle)-fabs(leftBorder.angle));
			}
		}
		else{
			...
			if(horiz_ctrl == END){
				if(fabs((float)leftPos.y-rightPos.y) > MIN_LANE_DISTANCE){
					if(leftPos.y<rightPos.y)
						sprintf(result,"Direction: Straight for %d then Left (Logic: Vertical + horiz)",minDist);
					else
						sprintf(result,"Direction: Straight for %d then Right (Logic: Vertical + horiz)",minDist);
				}
				else{
					if(maxDist-h_start.y >MIN_LANE_DISTANCE)
						sprintf(result,"Direction: Straight for %d then LEFT/RIGHT (Logic: Horiz line+ Open verical)",minDist);
					else
						sprintf(result,"Direction: Straight for %d then END (Logic: Horiz line+Closed Vertical)",minDist);
				}
			}
			else if(horiz_ctrl == RIGHT){
				sprintf(result,"Direction: Straight then Right");
			}
			else if(horiz_ctrl == LEFT){
				sprintf(result,"Direction: Straight then Left");
			}
		}
	}
	else if(leftLaneCount == 0 && rightLaneCount >0 && horizLaneCount ==0){		// Only right lanes
		sprintf(result,"Only right lane found");
	}
	else if(leftLaneCount > 0 && rightLaneCount  == 0 && horizLaneCount ==0){		// Only Left lanes
		sprintf(result,"Only left lane found");
	}
	else if(leftLaneCount == 0 && rightLaneCount  == 0 && horizLaneCount > 0){		// Only Left lanes
		sprintf(result,"Only Horizontal lane found. Control: Left/Right");
	}

	printf("%s\n",result);
	putText(raw, result, cvPoint(100,500), FONT_HERSHEY_COMPLEX, 0.8, cvScalar(0,0,255,255), 1, CV_AA);
	// Summary
	sprintf(result,"Horizontal lanes: %d",horizLaneCount);
	putText(raw, result, cvPoint(450,100), FONT_HERSHEY_COMPLEX, 0.8, cvScalar(0,255,0,255), 1.5, CV_AA);
	sprintf(result,"Vertical(left) lanes: %d",leftLaneCount);
	putText(raw, result, cvPoint(450,120), FONT_HERSHEY_COMPLEX, 0.8, cvScalar(0,255,0,255), 1.5, CV_AA);
	sprintf(result,"Vertical(right) lanes: %d",rightLaneCount);
	putText(raw, result, cvPoint(450,140), FONT_HERSHEY_COMPLEX, 0.8, cvScalar(0,255,0,255), 1.5, CV_AA);
	showIm("Filter-2 result",raw);
